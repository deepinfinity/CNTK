...
def create_reader(map_file, mean_file, train, distributed_after=cntk.io.INFINITE_SAMPLES):
...
def convnet_cifar10_dataaug(reader_train, reader_test, distributed_trainer, max_epochs=80, log_to_file=None, num_mbs_per_log=None, gen_heartbeat=False):
    ...
    lr_schedule      = cntk.learner.learning_rate_schedule(lr_per_sample, unit=cntk.learner.UnitType.sample, epoch_size=epoch_size)
    ...
    trainer = cntk.Trainer(z, ce, pe, learner, distributed_trainer)
    ...
    for epoch in range(max_epochs):       # loop over epochs
        ...
        while sample_count < epoch_size:  # loop over minibatches in the epoch
            ...
            progress_printer.update_with_trainer(trainer, with_metric=True) # generate minibatch progress to log

        progress_printer.epoch_summary(with_metric=True) # print epoch progreess to log
...
if __name__=='__main__':
    ...
    distributed_trainer = cntk.distributed.data_parallel_distributed_trainer(
    ...
    convnet_cifar10_dataaug(reader_train, reader_test, distributed_trainer, log_to_file="cba.log", num_mbs_per_log=10, gen_heartbeat=True)
